...
ASN="$(printf '%s' "$TRACE" | sed -n 's/^asn=//p')"
COL="$(printf '%s' "$TRACE" | sed -n 's/^colo=//p')"

printf '{"state":"up","ts":"%s","tun":"%s","ping_ms":"%s","egress_ip":"%s","country":"%s","asn":"%s","colo":"%s"}\n' \
	"$NOW" "$TUN" "$RTT" "$IP" "$CC" "$ASN" "$COL" >"$TMP" && mv "$TMP" "$OUT"